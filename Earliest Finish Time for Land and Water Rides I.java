...
class Earliest Finish Time for Land and Water Rides I{
    public int earliestFinishTime(int[] landStartTime, int[] landDuration,int[] waterStartTime, int[] waterDuration) {
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < m; j++) {
                ...
                int wFinish = wStart + waterDuration[j];
                int lStart = Math.max(wFinish, landStartTime[i]);
                int lFinish = lStart + landDuration[i];
                ans = Math.min(ans, lFinish);
            }
        }
        return ans;
    }
